...
namespace Sample.Client.BatchOrder
{
    class Program
    {
        ...
        {
            var host = "192.168.100.115:5000";
            try
            {
                var json = JsonConvert.SerializeObject(order);
                var data = new StringContent(json, Encoding.UTF8, "application/json");

                var responseMessage = await _client.PostAsync($"http://{host}/Order", data);
                ...
                if (responseMessage.StatusCode == HttpStatusCode.Accepted)
                {
                    ...
                    var orderAddress = $"http://{host}/Order?id={order.Id:D}";
